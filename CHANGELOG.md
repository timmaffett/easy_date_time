...
### Added
- Add day of week and month constants to `EasyDateTime`  (`EasyDateTime.monday` .. `EasyDateTime.january`) which correspond to the
  values of the `DateTime` class.  This allows for easier 'drop in' replacement of `DateTime` with `EasyDateTime`.
- Provide global configuration methods as static methods on `EasyDateTime` to provide option to set global configuration without
...
- Updated multilingual READMEs dependencies to `^0.3.1`.
